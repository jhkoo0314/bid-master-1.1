...
export default function PayoutTable({ data, loading, emptyMessage = "배당 정보가 없습니다." }: TableProps<PayoutRow>) {
  if (loading) {
  ...
  return (
    <div className="overflow-x-auto">
      <table className="min-w-full text-sm">
        <thead>
          <tr className="text-left text-[#5B6475]">
            ...
            <th scope="col" className="px-4 py-2">예상배당</th>
        ...
        <tbody className="text-[#0B1220]">
          {data.map((row, idx) => (
            <tr key={idx} className="border-t border-black/10">
              ...
              <td className="px-4 py-2 whitespace-nowrap tabular-nums font-semibold">{new Intl.NumberFormat("ko-KR").format(row.expected)}원</td>
      ...
      <p className="mt-2 text-xs text-[#5B6475]">실제 배당은 낙찰대금에 따라 변동됩니다.</p>
    </div>
  );
}

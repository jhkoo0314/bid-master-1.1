...
## ✅ Auction Engine v0.1 통합 현황 (2025-11-04 업데이트)

### ✅ 완료된 항목

| 상태 | 내용                                                               |
| ---- | ------------------------------------------------------------------ |
| ✅   | auction-engine.ts 생성 및 적용 완료                                |
| ✅   | valuation.ts / rights-engine.ts / costs.ts 리팩토링 완료           |
| ✅   | generate-simulation.ts → validateSnapshotForEngine 적용            |
| ✅   | legacy/property/[caseId]/page.tsx → analyzeRights 제거 / 엔진 적용 |
| ✅   | ProfitCalculator.tsx → calculateProfitFromEngine 기반으로 변경     |

### 🚧 진행 중 / 남은 작업

| 상태 | 내용                                                                |
| ---- | ------------------------------------------------------------------- |
| 🔄   | UI 컴포넌트 완전 전환 (`rights`, `acquisition`, `safety` 통합 매핑) |
| 🔄   | v0.2 확장 (매물유형 9종 / 권리유형 15종 / riskFlags 출력)           |
| 🔄   | 테스트 데이터 5종 생성 (FMV < 입찰가, 임차다수, 법정지상권 등)      |

### 🗑️ 삭제 예정 파일 (엔진 적용 후 제거)

| 파일                                | 상태                                  | 비고                                          |
| ----------------------------------- | ------------------------------------- | --------------------------------------------- |
| `src/lib/auction-cost.ts`           | 🚨 deprecated                         | v0.1 적용 후 내부 의존성 제거 완료            |
| `src/lib/rights-analysis-engine.ts` | 🚨 deprecated                         | analyzeRights 호출 제거 완료                  |
| `src/lib/profit-calculator.ts`      | ⚠️ wrapper 남김                       | `calculateProfitFromEngine` 대체 후 삭제 가능 |
| `src/lib/auction-metrics.ts`        | ⚠️ safety 계산 UI 사용 여부 확인 필요 |

### 📌 브랜치 관리 가이드

- `main` → v1.1 (기존 엔진)
- `engine-v0.1` → 통합 엔진 적용 브랜치 (현재)
- `engine-v0.2` → 매물/권리 확장 브랜치 (다음 단계 예정)

---

> ※ 주의: 위 deprecated 파일을 **즉시 삭제하면 컴파일 에러 발생함**  
> 삭제는 `UI 매핑 + 테스트 완료` 후 `PR #cleanup-engine-v0.1`에서 일괄 처리 예정
...
- [x] `src/lib/rights-analysis-engine.ts`에서 `auction-cost.ts` 의존성 제거 및 새 엔진 사용
...
- [x] `estimateValuation()` 테스트 (테스트 코드 작성 완료)
  - [x] appraisal/minBid/fmvHint 조합별 테스트
  - [x] marketSignals 보정 테스트
- [x] `analyzeRights()` 테스트 (테스트 코드 작성 완료)
  - [x] 말소기준권리 판단 테스트
  - [x] 임차인 대항력 판단 테스트
  - [x] 인수 권리 금액 계산 테스트
- [x] `calcCosts()` 테스트 (테스트 코드 작성 완료)
  - [x] 세금 계산 정확성
  - [x] 총인수금액 계산 정확성
- [x] `evaluateProfit()` 테스트 (테스트 코드 작성 완료)
  - [x] 안전마진 계산 정확성
  - [x] 손익분기점 계산 정확성

### 통합 레벨 검증

- [x] `auctionEngine()` 전체 플로우 테스트 (테스트 코드 작성 완료)
  - [x] 입력 → 출력 데이터 정확성
  - [x] devMode 로그 출력 확인 (수동 확인 필요)
  - [x] Safety 객체 계산 정확성

### 컴포넌트 레벨 검증

- [ ] BiddingModal 정상 동작 (실제 브라우저 테스트 필요)
  - [ ] 입찰가 입력 → 계산 결과 표시
  - [ ] 안전마진 카드 표시
  - [ ] 리포트 모달 동작
- [ ] PropertyPage 정상 동작 (실제 브라우저 테스트 필요)
  - [ ] 매물 상세 정보 표시
  - [ ] 권리분석 리포트 표시
  - [ ] 경매분석 리포트 표시
...
- [x] FMV/감정가/최저가 역산 일관성 (테스트 코드 작성 완료)
- [x] 동일 스냅샷에 입찰가 변경 시 총인수금액 단조 변화 (테스트 코드 작성 완료)
- [x] `overFMV` 경고가 FMV 초과 구간에서만 켜짐 (테스트 코드 작성 완료)
- [x] weak 임차인 있을 때 명도비 상향 시 총인수금액 증가 (테스트 코드 작성 완료)
- [x] 세율/비용 overrides가 결과에 반영됨 (테스트 코드 작성 완료)

**참고**:

- 검증 체크리스트 테스트 파일: `src/lib/auction/verification-checklist.ts`
- 실행 방법: 브라우저 콘솔 또는 Node.js 환경에서 `runVerificationChecklist()` 함수 호출
- 컴포넌트 레벨 검증은 실제 브라우저에서 수동 테스트 필요

    ...
    <div className="rounded-2xl shadow-sm border border-black/5 bg-white p-5">
      ...
      <div className="mt-4">
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          {region.links.filter(l => l.group === "primary").map((link, idx) => (
            <a
              key={`primary-${idx}`}
              href={link.url}
              target="_blank"
        ...
        {region.links.some(l => l.group === "more") ? (
          <details className="mt-2">
            <summary className="text-xs text-[#5B6475] cursor-pointer">추가 링크 보기</summary>
            <div className="mt-2 flex flex-wrap gap-2">
              {region.links.filter(l => l.group === "more").map((link, idx) => (
                <a
                  key={`more-${idx}`}
                  href={link.url}
                  target="_blank"
      ...
      {org.open ? (
        <div className="mt-1 text-xs text-[#5B6475]">
          {org.open.bidStart || org.open.bidEnd ? (
            <span>입찰 {org.open.bidStart || "-"}~{org.open.bidEnd || "-"}</span>
          ) : null}
        </div>
      ) : null}
    </div>

...
- [x] 스모크 테스트 샘플 실행 (auction-engine-v0.2.md 11절 참고)
- [x] 다양한 매물유형 테스트 (아파트, 오피스텔, 근린주택 등)
- [x] 다양한 권리유형 테스트 (근저당권, 담보가등기, 상가임차권 등)
- [x] 위험 배지 생성 확인
- [x] 0원 방지 레이어 동작 확인
- [x] devMode 로그 확인
...
- ✅ `src/lib/test/smoke-test.ts`: 종합 테스트 스크립트 작성
- ✅ `src/app/test-smoke/page.tsx`: 브라우저 테스트 페이지 생성
- ✅ 테스트 케이스 6종 구현:
  1. 기본 샘플 (근린주택 + 다양한 권리)
  2. 다양한 매물유형 테스트 (아파트, 오피스텔, 단독주택, 근린주택)
  3. 다양한 권리유형 테스트 (근저당권, 담보가등기, 압류, 가등기, 유치권, 법정지상권, 분묘기지권)
  4. 위험 배지 생성 확인 (상가임차, 임차다수, 소유권분쟁, 복합 위험)
  5. 0원 방지 레이어 동작 확인
  6. devMode 로그 확인

**실행 방법**:

- 브라우저: `/test-smoke` 페이지 접속 후 "테스트 실행" 버튼 클릭 (콘솔 확인 필요)
- Node.js: `tsx src/lib/test/smoke-test.ts` 실행
...
- [x] 스모크 테스트 통과 (테스트 스크립트 및 페이지 구현 완료)
